...
BoardDrawingScene::BoardDrawingScene() : pieceMovingData(*this) {}
...
void BoardDrawingScene::onSizeChanged(Point size)
{
	...
	paneRect = { paneStartX, boardRect.top - BoardBorder,
				paneStartX + RightPaneWidth, boardRect.bottom + BoardBorder };
}
...
void BoardDrawingScene::drawEatenPieces(Paint& p) const
{
	...
	int startX = (paneRect.width() - pieceSize * 8 - (fullPieceSize - pieceSize)) / 2;
	...
	drawPieces(botPos, getBoard().getEatenPieces(topSide));
	drawPieces(topPos, getBoard().getEatenPieces(botSide));
}
...
void BoardDrawingScene::PieceMovingData::setInterpolatedMove(chess::FullMove move)
{
...
void BoardDrawingScene::PieceMovingData::setMousePos(Point p, chess::Pos boardPos)
{
