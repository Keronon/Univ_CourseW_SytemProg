...
constexpr int lerp(int a, int b, double t) { return int(a + t * (b - a)); }

EndGameScene::EndGameScene() : MenuScene(
		{
...
{
	return GameScene::instance().getBoard();
}
const std::string& EndGameScene::getPlayerName(chess::Side s) const
{
	return GameScene::instance().getPlayerName(s);
}
chess::Side EndGameScene::getPlayerSide() const
{
	return GameScene::instance().getPlayerSide();
}
...
void EndGameScene::onDraw(Paint& p)
{
	switch (state)
	{
		...
		case State::ShowingAnimation:
		{
			...
			{
				int size = lerp(TitleSizeSmall, TitleSizeBig, (dt - waitMs) / durationMs);
...
void EndGameScene::loadImpl(chess::FullMove move, std::string_view title,
