...
{
	int i = 0;
	for (auto& d : this->buttons) d.rect_ = &getButtonRect(i++);
}

void MenuScene::updateVertSizes(Rect rect)
{
	getButtonRects().updateEquallySpacedVert(rect, VertButtonSize, ButtonSpacing);
}

void MenuScene::onDrawBackground(Paint& p)
{
	for (auto& r : getButtonRects()) p.fillRect(r, ButtonCol);
}
...
void MenuScene::onDraw(core::Paint& p)
{
	...
	for (auto& b : buttons)
	{
		...
		{
			[&](const ButtonData::Normal&) { p.drawText(b.rect(), b.text); },
			[&](const ButtonData::LeftRight& lr)
			{
				auto val = lr.getVal();
				p.drawText(b.rect(), concat(b.text, ": ", val));
				drawLeft(p, b.rect(), sprites::Left, getPalette(val != lr.min));
				drawRight(p, b.rect(), sprites::Right, getPalette(val != lr.max));
			},
...
void MenuScene::onButtonSelected(int index)
{
	...
	std::visit([&](auto&& arg)
	{
		...
		if constexpr (std::is_same_v<T, ButtonData::LeftRight>)
		{
			onButtonFocusMove(index, index);
		}
	}, b.data);
}

void MenuScene::onButtonFocusMove(int index, int delta)
{
	auto& b = getButton(index);
	std::visit([&](auto&& arg)
	{
		...
		else if constexpr (std::is_same_v<T, ButtonData::Radio> || std::is_same_v<T, ButtonData::SpriteRadio>)
		{
...
void MenuScene::drawRight(core::Paint& p, const core::Rect& r,
	                      ...
	                      const core::Palette& palette) const
{
	p.drawSprite(r.topRight() - core::Point{ r.height(), 0 }, sprite, palette,
