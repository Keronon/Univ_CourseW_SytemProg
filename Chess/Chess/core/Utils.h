...
namespace core
{
	...
	public:
		explicit WinapiError(std::string_view msg = "", const source_location& location = source_location::current())
	...
	private:
		static std::string makeMessage(std::string_view msg, DWORD& code, const source_location& location);
	};
	...
	struct Point
	{
		...
		constexpr friend Point operator*(Point a, int b) { return b * a; }
		...
		constexpr int64_t length2() const { return x * x + y * y; }
	};
	...
	struct Rect
	{
		...
		{}
		constexpr Rect(RECT val) : Rect(val.left, val.top, val.right, val.bottom) {}
		...
		constexpr operator RECT() const { return { left, top, right, bottom }; }
		...
		constexpr Rect& operator-=(Point p) { return *this += -p; }
	...
	public:
		WindowDC(HWND hwnd) : val(::GetDC(hwnd)), hwnd(hwnd)
		{
			if (val == nullptr) throw WinapiError("GetDC");
		}
		...
		~WindowDC() noexcept { ::ReleaseDC(hwnd, val); }
	};
}
